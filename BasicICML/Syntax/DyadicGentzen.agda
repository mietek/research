...
mutual
  ...
  data _⊢_ : Cx² Ty Box → Ty → Set where
    ...
    mvar  : ∀ {A Ψ Γ Δ}   → [ Ψ ] A ∈ Δ → {{_ : Γ ⁏ Δ ⊢⋆ Ψ}} → Γ ⁏ Δ ⊢ A
    box   : ∀ {A Ψ Γ Δ}   → Ψ ⁏ Δ ⊢ A → Γ ⁏ Δ ⊢ [ Ψ ] A
    unbox : ∀ {A C Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A → Γ ⁏ Δ , [ Ψ ] A ⊢ C → Γ ⁏ Δ ⊢ C
  ...
  data _⊢⋆_ : Cx² Ty Box → Cx Ty → Set where
    instance
      ∙   : ∀ {Γ Δ}     → Γ ⁏ Δ ⊢⋆ ∅
      _,_ : ∀ {Γ Δ Ξ A} → Γ ⁏ Δ ⊢⋆ Ξ → Γ ⁏ Δ ⊢ A → Γ ⁏ Δ ⊢⋆ Ξ , A
...
mv₀ : ∀ {A Ψ Γ Δ} → {{_ : Γ ⁏ Δ , [ Ψ ] A ⊢⋆ Ψ}}
                  → Γ ⁏ Δ , [ Ψ ] A ⊢ A
mv₀ = mvar i₀

mv₁ : ∀ {A B Ψ Ψ′ Γ Δ} → {{_ : Γ ⁏ Δ , [ Ψ ] A , [ Ψ′ ] B ⊢⋆ Ψ}}
                       → Γ ⁏ Δ , [ Ψ ] A , [ Ψ′ ] B ⊢ A
mv₁ = mvar i₁

mv₂ : ∀ {A B C Ψ Ψ′ Ψ″ Γ Δ} → {{_ : Γ ⁏ Δ , [ Ψ ] A , [ Ψ′ ] B , [ Ψ″ ] C ⊢⋆ Ψ}}
                            → Γ ⁏ Δ , [ Ψ ] A , [ Ψ′ ] B , [ Ψ″ ] C ⊢ A
mv₂ = mvar i₂
...
mlam : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ , [ Ψ ] A ⊢ B → Γ ⁏ Δ ⊢ [ Ψ ] A ▻ B
...
mdet : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A ▻ B → Γ ⁏ Δ , [ Ψ ] A ⊢ B
...
mcut : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A → Γ ⁏ Δ , [ Ψ ] A ⊢ B → Γ ⁏ Δ ⊢ B
...
mcont : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ , [ Ψ ] A , [ Ψ ] A ⊢ B → Γ ⁏ Δ , [ Ψ ] A ⊢ B
...
mexch : ∀ {A B C Ψ Ψ′ Γ Δ} → Γ ⁏ Δ , [ Ψ ] A , [ Ψ′ ] B ⊢ C → Γ ⁏ Δ , [ Ψ′ ] B , [ Ψ ] A ⊢ C
...
mcomp : ∀ {A B C Ψ Ψ′ Ψ″ Γ Δ} → Γ ⁏ Δ , [ Ψ′ ] B ⊢ [ Ψ″ ] C → Γ ⁏ Δ , [ Ψ ] A ⊢ [ Ψ′ ] B → Γ ⁏ Δ , [ Ψ ] A ⊢ [ Ψ″ ] C
mcomp t u = mdet (app (app ccomp (mlam t)) (mlam u))


-- Useful theorems in functional form.

dist : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] (A ▻ B) → Γ ⁏ Δ ⊢ [ Ψ ] A → Γ ⁏ Δ ⊢ [ Ψ ] B
dist t u = unbox t (unbox (mmono⊢ weak⊆ u) (box (app mv₁ mv₀)))

up : ∀ {A Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A → Γ ⁏ Δ ⊢ [ Ψ ] [ Ψ ] A
up t = unbox t (box (box mv₀))

down : ∀ {A Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A → {{_ : Γ ⁏ Δ , [ Ψ ] A ⊢⋆ Ψ}} → Γ ⁏ Δ ⊢ A
down t = unbox t mv₀

distup : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] ([ Ψ ] A ▻ B) → Γ ⁏ Δ ⊢ [ Ψ ] A → Γ ⁏ Δ ⊢ [ Ψ ] B
...
cdist : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] (A ▻ B) ▻ [ Ψ ] A ▻ [ Ψ ] B
cdist = lam (lam (dist v₁ v₀))

cup : ∀ {A Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A ▻ [ Ψ ] [ Ψ ] A
cup = lam (up v₀)

cdistup : ∀ {A B Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] ([ Ψ ] A ▻ B) ▻ [ Ψ ] A ▻ [ Ψ ] B
cdistup = lam (lam (dist v₁ (up v₀)))

cunbox : ∀ {A C Ψ Γ Δ} → Γ ⁏ Δ ⊢ [ Ψ ] A ▻ ([ Ψ ] A ▻ C) ▻ C
...
mconcat : ∀ {A B Ψ Γ Δ} Δ′ → Γ ⁏ Δ , [ Ψ ] A ⊢ B → Γ ⁏ Δ′ ⊢ [ Ψ ] A → Γ ⁏ Δ ⧺ Δ′ ⊢ B
...
mutual
  m[_≔_]_ : ∀ {A C Ψ Γ Δ} → (i : [ Ψ ] A ∈ Δ) → Ψ ⁏ Δ ∖ i ⊢ A → Γ ⁏ Δ ⊢ C → Γ ⁏ Δ ∖ i ⊢ C
  m[ i ≔ s ] var j          = var j
  ...
  m[_≔_]⋆_ : ∀ {Ξ A Ψ Γ Δ} → (i : [ Ψ ] A ∈ Δ) → Ψ ⁏ Δ ∖ i ⊢ A → Γ ⁏ Δ ⊢⋆ Ξ → Γ ⁏ Δ ∖ i ⊢⋆ Ξ
  m[_≔_]⋆_ {∅}     i s ∙        = ∙
...
data _⋙_ {Δ : Cx Box} {Γ : Cx Ty} : ∀ {A} → Γ ⁏ Δ ⊢ A → Γ ⁏ Δ ⊢ A → Set where
  ...
  -- congbox⋙   : ∀ {A Ψ} → {t t′ : Ψ ⁏ Δ ⊢ A}
  --                       → t ⋙ t′
  --                       → box t ⋙ box t′

  congunbox⋙ : ∀ {A C Ψ} → {t t′ : Γ ⁏ Δ ⊢ [ Ψ ] A} → {u u′ : Γ ⁏ Δ , [ Ψ ] A ⊢ C}
                          → t ⋙ t′ → u ⋙ u′
                          → unbox t u ⋙ unbox t′ u′
  ...
  beta□⋙     : ∀ {A C Ψ} → {t : Ψ ⁏ Δ ⊢ A} → {u : Γ ⁏ Δ , [ Ψ ] A ⊢ C}
...
module Examples where
  e₁ : ∀ {A C D Γ Δ} → Γ ⁏ Δ ⊢ [ ∅ , C ] A ▻ [ ∅ , C , D ] A
  e₁ = lam (unbox v₀
         (box mv₀))

  e₂ : ∀ {A C Γ Δ} → Γ ⁏ Δ ⊢ [ ∅ , C , C ] A ▻ [ ∅ , C ] A
  e₂ = lam (unbox v₀
         (box mv₀))
  ...
  e₆ : ∀ {A C D Γ Δ} → Γ ⁏ Δ ⊢ [ ∅ , C ] A ▻ [ ∅ , D ] [ ∅ , C ] A
  e₆ = lam (unbox v₀ (box (box mv₀)))

  e₇ : ∀ {A B C D Γ Δ} → Γ ⁏ Δ ⊢ [ ∅ , C ] (A ▻ B) ▻ [ ∅ , D ] A ▻ [ ∅ , C , D ] B
  e₇ = lam (lam (unbox v₁ (unbox v₀
         (box (app mv₁ mv₀)))))

  e₈ : ∀ {A B C Γ Δ} → Γ ⁏ Δ ⊢ [ ∅ , A ] (A ▻ B) ▻ [ ∅ , B ] C ▻ [ ∅ , A ] C
  e₈ = lam (lam (unbox v₁ (unbox v₀
         (box (mv₀ {{∙ , app mv₁ v₀}})))))
