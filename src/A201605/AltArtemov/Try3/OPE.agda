module A201605.AltArtemov.Try3.OPE where

open import A201605.AltArtemov.Try3.Vec public
open import A201605.AltArtemov.Try3.Cx public


data _⊇_ : Cx → Cx → Set where
  base : ∅ ⊇ ∅
  weak : ∀ {Γ Γ′ k} {A : Ty k} → Γ′ ⊇ Γ → (Γ′ , A) ⊇ Γ
  lift : ∀ {Γ Γ′ k} {A : Ty k} → Γ′ ⊇ Γ → (Γ′ , A) ⊇ (Γ , A)
...
⊇str : ∀ {Γ Γ′ k} {A : Ty k} → Γ′ ⊇ (Γ , A) → Γ′ ⊇ Γ
⊇str (weak η) = weak (⊇str η)
⊇str (lift η) = weak η

⊇drop : ∀ {Γ Γ′ k} {A : Ty k} → (Γ′ , A) ⊇ (Γ , A) → Γ′ ⊇ Γ
⊇drop (weak η) = ⊇str η
⊇drop (lift η) = η
...
°ren-tm-id : ∀ {Γ k} (t : Tm ᵍ⌊ Γ ⌋ k) → ren-tm ʰ⌊ ⊇id ⌋ t ≡ t
