...
data Prov (Γ : Cx) : ∀ {k n m} → Vec ᵍ⌊ Γ ⌋ k n → Ty m → Set where
  ...
  lam  : ∀ {k k′ n} {A : Ty k} {ts : Vec (suc ᵍ⌊ Γ ⌋) k′ n} {B : Ty k′} →
           Prov (Γ , A) ts B →
           Prov Γ (LAMs ts) (A ⊃ B)
  app  : ∀ {k k′ n} {ts₁ : Vec ᵍ⌊ Γ ⌋ 0 n} {ts₂ : Vec ᵍ⌊ Γ ⌋ k n} {A : Ty k} {B : Ty k′} →
           Prov Γ ts₁ (A ⊃ B) → Prov Γ ts₂ A →
           Prov Γ (APPs {!ts₁!} ts₂) B
  pair : ∀ {k k′ n} {ts₁ : Vec ᵍ⌊ Γ ⌋ k n} {A : Ty k} {ts₂ : Vec ᵍ⌊ Γ ⌋ k′ n} {B : Ty k′} →
           Prov Γ ts₁ A → Prov Γ ts₂ B →
           Prov Γ (PAIRs {!ts₁!} ts₂) (A ∧ B)
  fst  : ∀ {k k′ n} {ts : Vec ᵍ⌊ Γ ⌋ 0 n} {A : Ty k} {B : Ty k′} →
           Prov Γ ts (A ∧ B) →
           Prov Γ (FSTs ts) A
  snd  : ∀ {k k′ n} {ts : Vec ᵍ⌊ Γ ⌋ 0 n} {A : Ty k} {B : Ty k′} →
           Prov Γ ts (A ∧ B) →
           Prov Γ (SNDs ts) B
  up   : ∀ {k n} {ts : Vec ᵍ⌊ Γ ⌋ (suc k) n} {u : Tm 0 k} {A : Ty k} →
           Prov Γ ts (u ∶ A) →
           Prov Γ (UPs ts) ((! u ∶ u ∶ A))
  down : ∀ {k n} {ts : Vec ᵍ⌊ Γ ⌋ (suc k) n} {u : Tm 0 k} {A : Ty k} →
           Prov Γ ts (u ∶ A) →
           Prov Γ (DOWNs ts) A

--ᵗ⌊_⌋ : ∀ {Γ k n m} {A : Ty k} {ts : Vec ᵍ⌊ Γ ⌋ k n} → Prov Γ ts A → Tm ᵍ⌊ Γ ⌋ m
--ᵗ⌊ var x ⌋      = VAR ⁱ⌊ x ⌋
--ᵗ⌊ lam j ⌋      = LAM ᵗ⌊ j ⌋
...
-- true⇒ (snd j)      = snd {!true⇒ j!} -- snd (true⇒ j)
-- true⇒ (up j)       = up (true⇒ j)
