...
mono[⊢] : ∀ {Γ Δ Γ′ Δ′ Ψ A} → Δ′ ⊇ Δ → Γ′ ⊇ Γ → Δ ⁏ Γ [⊢] [ Ψ ] A → Δ′ ⁏ Γ′ [⊢] [ Ψ ] A
...
mrefl[⊢]⋆ : ∀ {Γ Δ} → Δ ⁏ Γ [⊢]⋆ Δ
mrefl[⊢]⋆ {Γ} {∅}           = ∅
mrefl[⊢]⋆ {Γ} {Δ , [ Ψ ] A} = mono[⊢]⋆ {Γ} (weak refl⊇) refl⊇ (mrefl[⊢]⋆ {Γ}) , mvar refl⊢⋆ zero

mlookup[⊢] : ∀ {Γ Δ Ξ Ψ A} → Δ ⁏ Γ [⊢]⋆ Ξ → Ξ ∋ [ Ψ ] A → Δ ⁏ Γ [⊢] [ Ψ ] A
mlookup[⊢] ξ 𝒾 = lookupAll ξ 𝒾

mutual
  mgraft[⊢] : ∀ {Γ Δ Φ Ψ A} → Δ ⁏ Γ [⊢]⋆ Φ → Φ ⁏ Γ [⊢] [ Ψ ] A → Δ ⁏ Γ [⊢] [ Ψ ] A
  mgraft[⊢] {Γ} φ (var 𝒾)     = var 𝒾
  mgraft[⊢] {Γ} φ (mvar ψ 𝒾)  = graft⊢ (mgraft⊢⋆ {Γ} φ ψ) (mlookup[⊢] {Γ} φ 𝒾)
  mgraft[⊢] {Γ} φ (lam 𝒟)     = lam (mgraft[⊢] {Γ} φ 𝒟)
  mgraft[⊢] {Γ} φ (app 𝒟 ℰ)   = app (mgraft[⊢] {Γ} φ 𝒟) (mgraft[⊢] {Γ} φ ℰ)
  mgraft[⊢] {Γ} φ (box 𝒟)     = box (mgraft[⊢] {Γ} φ 𝒟)
  mgraft[⊢] {Γ} φ (unbox 𝒟 ℰ) = unbox (mgraft[⊢] {Γ} φ 𝒟)
                                       (mgraft[⊢] {Γ} (mono[⊢]⋆ {Γ} (weak refl⊇) refl⊇ φ , mvar refl⊢⋆ zero) ℰ)

  mgraft⊢⋆ : ∀ {Γ Δ Φ Ψ Ξ} → Δ ⁏ Γ [⊢]⋆ Φ → Φ ⁏ Ψ ⊢⋆ Ξ → Δ ⁏ Ψ ⊢⋆ Ξ
  mgraft⊢⋆ {Γ} φ ∅       = ∅
  mgraft⊢⋆ {Γ} φ (ξ , 𝒟) = mgraft⊢⋆ {Γ} φ ξ , mgraft[⊢] {Γ} φ 𝒟
  -- NOTE: Equivalent, but does not pass termination check.
  -- mgraft[⊢]⋆ {Γ} φ ξ = mapAll (mgraft[⊢] φ) ξ

mgraft[⊢]⋆ : ∀ {Γ Δ Φ Ξ} → Δ ⁏ Γ [⊢]⋆ Φ → Φ ⁏ Γ [⊢]⋆ Ξ → Δ ⁏ Γ [⊢]⋆ Ξ
mgraft[⊢]⋆ {Γ} φ ξ = mapAll (mgraft[⊢] {Γ} φ) ξ

mtrans[⊢]⋆ : ∀ {Γ Δ Δ′ Δ″} → Δ″ ⁏ Γ [⊢]⋆ Δ′ → Δ′ ⁏ Γ [⊢]⋆ Δ → Δ″ ⁏ Γ [⊢]⋆ Δ
mtrans[⊢]⋆ {Γ} = mgraft[⊢]⋆ {Γ}
