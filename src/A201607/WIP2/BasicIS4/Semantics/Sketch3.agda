...
eval (var i)             γ Δ δ = lookup i γ
eval (lam {A} {B} t)     γ Δ δ = return {A ▻ B} λ { (η , θ) a →
                                   eval t (mono⊩⋆ (η , θ) γ , a) (mmono⊢⋆ θ Δ) (mono⊩⋆ (refl⊆ , θ) δ) }
eval (app {A} {B} t u)   γ Δ δ = bind {A ▻ B} {B} (eval t γ Δ δ) λ { (η , θ) f →
                                   _⟪$⟫_ {A} {B} f (eval u (mono⊩⋆ (η , θ) γ) (mmono⊢⋆ θ Δ) (mono⊩⋆ (refl⊆ , θ) δ)) }
eval (mvar {A} i)        γ Δ δ = mono⊩ {A} (bot⊆ , refl⊆) (lookup i δ)
eval (box {A} t)         γ Δ δ = return {□ A} λ θ →
                                   mono⊢ bot⊆ (mmulticut′ (mmono⊢⋆ θ Δ) t) , eval t ∙ (mmono⊢⋆ θ Δ) (mono⊩⋆ (refl⊆ , θ) δ)
eval (unbox {A} {C} t u) γ Δ δ = bind {□ A} {C} (eval t γ Δ δ) λ { (η , θ) a →
                                   eval u (mono⊩⋆ (η , θ) γ) (mmono⊢⋆ θ Δ , π₁ (a refl⊆)) (mono⊩⋆ (refl⊆ , θ) δ , π₂ (a refl⊆)) }
eval (pair {A} {B} t u)  γ Δ δ = return {A ∧ B} (eval t γ Δ δ , eval u γ Δ δ)
eval (fst {A} {B} t)     γ Δ δ = bind {A ∧ B} {A} (eval t γ Δ δ) (K π₁)
eval (snd {A} {B} t)     γ Δ δ = bind {A ∧ B} {B} (eval t γ Δ δ) (K π₂)
eval unit                γ Δ δ = return {⊤} ∙

eval⋆ : ∀ {Ξ Γ} → Γ ⊢⋆ Ξ → Γ ⊨⋆ Ξ
eval⋆ {∅}     ∙        γ Δ δ = ∙
eval⋆ {Ξ , A} (ts , t) γ Δ δ = eval⋆ ts γ Δ δ , eval t γ Δ δ
