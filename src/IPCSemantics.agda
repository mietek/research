...
idmono⊩ : ∀ {{_ : Model}} {A w} → (a : w ⊩ A) → mono⊩ {A} refl⊒ a ≡ a
idmono⊩ {•}      a = idmonoG a
idmono⊩ {A ⇒ B} a = fex′ (fex (λ η → cong a (idtrans⊒₂ η)))

assocmono⊩ : ∀ {{_ : Model}} {A w w′ w″} → (η′ : w″ ⊒ w′) (η : w′ ⊒ w) (a : w ⊩ A) →
...
module IPCSemanticsList where
  ...
  idmono⊩⋆ : ∀ {{_ : Model}} {w Ξ} → (ξ : w ⊩⋆ Ξ) → mono⊩⋆ refl⊒ ξ ≡ ξ
  idmono⊩⋆ {Ξ = ∅}     ∅       = refl
  idmono⊩⋆ {Ξ = Ξ , A} (ξ , a) = cong² _,_ (idmono⊩⋆ ξ) (idmono⊩ {A} a)

  assocmono⊩⋆ : ∀ {{_ : Model}} {w w′ w″ Ξ} → (η′ : w″ ⊒ w′) (η : w′ ⊒ w) (ξ : w ⊩⋆ Ξ) →
...
module IPCSemanticsVec where
  ...
  idmono⊩⋆ : ∀ {{_ : Model}} {w n} {Ξ : Ty⋆ n} →
                (ξ : w ⊩⋆ Ξ) → mono⊩⋆ refl⊒ ξ ≡ ξ
  idmono⊩⋆ {Ξ = ∅}     ∅       = refl
  idmono⊩⋆ {Ξ = Ξ , A} (ξ , a) = cong² _,_ (idmono⊩⋆ ξ) (idmono⊩ {A} a)

  assocmono⊩⋆ : ∀ {{_ : Model}} {w w′ w″ n} {Ξ : Ty⋆ n} →
                   (η′ : w″ ⊒ w′) (η : w′ ⊒ w) (ξ : w ⊩⋆ Ξ) →
                   mono⊩⋆ η′ (mono⊩⋆ η ξ) ≡ mono⊩⋆ (trans⊒ η′ η) ξ
  assocmono⊩⋆ {Ξ = ∅}     η′ η ∅       = refl
  assocmono⊩⋆ {Ξ = Ξ , A} η′ η (ξ , a) = cong² _,_ (assocmono⊩⋆ η′ η ξ) (assocmono⊩ {A} η′ η a)
