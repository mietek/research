...
infix 3 _⨾_
data _⨾_ : List Validity → Derivability → Set
  where
    var : ∀ {A Δ Γ} → Γ ∋ A true
                    → Δ ⨾ Γ ⊢ A true
...
pac : List Truth → List Truth → List Derivability
pac Γ ∙            = ∙
pac Γ (Ξ , A true) = pac Γ Ξ , (Γ ⊢ A true)
...
infix 3 _⨾⋆_
_⨾⋆_ : List Validity → Derivabilities → Set
Δ ⨾⋆ Γ ⊢⋆ Ξ = All (Δ ⨾_) (pac Γ Ξ)
...
rens : ∀ {Δ Γ Γ′ Ξ} → Γ′ ⊇ Γ → Δ ⨾⋆ Γ ⊢⋆ Ξ
                    → Δ ⨾⋆ Γ′ ⊢⋆ Ξ
rens {Ξ = ∙}          η ∙       = ∙
rens {Ξ = Ξ , A true} η (ξ , 𝒟) = rens η ξ , ren η 𝒟
-- NOTE: Equivalent to
...
subs : ∀ {Δ Γ Ξ Ψ} → Δ ⨾⋆ Γ ⊢⋆ Ξ → Δ ⨾⋆ Ξ ⊢⋆ Ψ
                   → Δ ⨾⋆ Γ ⊢⋆ Ψ
subs {Ψ = ∙}          ξ ∙       = ∙
subs {Ψ = Ξ , A true} ξ (ψ , 𝒟) = subs ξ ψ , sub ξ 𝒟
-- NOTE: Equivalent to
