...
module UniqueList where
  ...
  module DecUniqueList {ℓ} {X : Set ℓ} (_≡?_ : (x y : X) → Dec (x ≡ y)) where
    ...
    lem-length : ∀ xs ys → length (xs ∪ ys) ≤ length xs + length ys -- TODO: find a better name
    lem-length []       ys = ≤-refl
    lem-length (x ∷ xs) ys with ys ∌? x
    ...                    | yes ys∌x = s≤s (lem-length xs ys)
    ...                    | no ys∋x  = ≤-step (lem-length xs ys)
...
lem-333 (if t₁ then t₂ else t₃) = ≤-step
    ...
    ≤⟨ lem-length (consts t₁ ∪ consts t₂) (consts t₃) ⟩
      length (consts t₁ ∪ consts t₂) + length (consts t₃)
    ≤⟨ +-monoˡ-≤ (length (consts t₃)) (lem-length (consts t₁) (consts t₂)) ⟩
