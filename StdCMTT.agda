...
pac∋ : ∀ {d g n i A} → {σ : Scopes d} {Γ : Truths g}
...
pac₁ : ∀ {d n} → {σ : Scopes d} {ξ : Scopes n}
               → Terms₁ σ ξ → Validities ξ
               → Vec (Derivation₁ σ) n
pac₁ ∙       ∙                  = ∙
pac₁ (x , M) (Ξ , A valid[ Ψ ]) = pac₁ x Ξ , (∙⊢₁ M ⦂ A valid[ Ψ ])

pac∋₁ : ∀ {m d n i A} → {Ψ : Truths m} {σ : Scopes d} {ξ : Scopes n}
                         {x : Terms₁ σ ξ} {Ξ : Validities ξ} {𝒾 : ξ ∋⟨ i ⟩ m}
                      → Ξ ∋◇⟨ 𝒾 ⟩ A valid[ Ψ ]
                      → pac₁ x Ξ ∋⟨ i ⟩ (∙⊢₁ get x 𝒾 ⦂ A valid[ Ψ ])
pac∋₁ {x = x , M} {Ξ , A valid[ Ψ ]} zero    = zero
pac∋₁ {x = x , N} {Ξ , B valid[ Y ]} (suc 𝒾) = suc (pac∋₁ 𝒾)
...
mutual
  ...
  msub ξ (mvar `𝒾 ψ)  = sub (msubs ξ ψ) (get ξ (pac∋₁ `𝒾))
