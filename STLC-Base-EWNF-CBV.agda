...
sub¬Expandable : ∀ {A Γ Ξ} {ss : Ξ ⊢* Γ} {t : Γ ⊢ A} → FNNF* ss → ¬ Expandable t →
                 ¬ Expandable (sub ss t)
sub¬Expandable {A ⌜⊃⌝ B} {t = var i}     ps ¬x x           = var- ↯ ¬x
sub¬Expandable {A ⌜⊃⌝ B} {t = t₁ ⌜$⌝ t₂} ps ¬x (p₁ ⌜$⌝ p₂) = (busFNNF ps p₁ ⌜$⌝ busFNF ps p₂) ↯ ¬x
...
Expandable? {A ⌜⊃⌝ B} (t₁ ⌜$⌝ t₂) with FNNF? t₁ | FNF? t₂
... | no ¬p₁ | _                    = no λ { (p₁ ⌜$⌝ p₂) → p₁ ↯ ¬p₁ }
