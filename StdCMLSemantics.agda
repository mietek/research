...
module _ {{_ : Model}}
  where
    ...
    syn : ∀ {A Ψ W} → W ⊪₁ A valid[ Ψ ]
                    → ⌊ W ⌋₁ ⊢₁ A valid[ Ψ ]
    syn v = proj₁ v

    sem : ∀ {A Ψ W} → W ⊪₁ A valid[ Ψ ]
                    → (∀ {W′} → W′ ≥ W → W′ ⊪⋆ Ψ
                               → W′ ⊪ A true)
    sem v = proj₂ v
    ...
    mutual
      ...
      crel : ∀ {Aₜ W W′} → W′ ≥ W → W ⊪ Aₜ
                         → W′ ⊪ Aₜ
      crel η k = \ η′ f → k (η ∘≥ η′) f

      crel₁ : ∀ {Aᵥ W W′} → W′ ≥ W → W ⊪₁ Aᵥ
                          → W′ ⊪₁ Aᵥ
    ...
    bind : ∀ {A B W} → W ⊪ A true → (∀ {W′} → W′ ≥ W → W′ ⊩ A true
                                               → W′ ⊪ B true)
                     → W ⊪ B true
    bind k f = \ η f′ →
                 k η (\ η′ a →
