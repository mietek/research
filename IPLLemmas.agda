...
comp-wks-rens-keep : ∀ {Γ Γ′ Ξ A} → (η : Γ′ ⊇ Γ) (ξ : Γ ⊢ Ξ true*)
...
comp-lifts-rens : ∀ {Γ Γ′ Ξ A} → (η : Γ′ ⊇ Γ) (ξ : Γ ⊢ Ξ true*)
...
comp-rens-gets : ∀ {Γ Γ′ Ξ Ξ′} → (η₁ : Γ′ ⊇ Γ) (ξ : Γ ⊢ Ξ′ true*) (η₂ : Ξ′ ⊇ Ξ)
...
comp-lifts-gets : ∀ {Γ Ξ Ξ′ A} → (ξ : Γ ⊢ Ξ′ true*) (η : Ξ′ ⊇ Ξ)
...
comp-sub-get : ∀ {Γ Ξ Ψ A} → (ξ : Γ ⊢ Ξ true*) (ψ : Ξ ⊢ Ψ true*) (i : Ψ ∋ A)
...
comp-sub-ren : ∀ {Γ Ξ Ξ′ A} → (ξ : Γ ⊢ Ξ′ true*) (η : Ξ′ ⊇ Ξ) (𝒟 : Ξ ⊢ A true)
...
expand-sub : ∀ {Γ Ξ A B} → (ξ : Γ ⊢ Ξ true*) (𝒟 : Γ ⊢ A true) (ℰ : Ξ ⊢ B true)
                         → sub (ξ , 𝒟) (wk ℰ) ≡ sub ξ ℰ
expand-sub ξ 𝒟 ℰ = comp-sub-ren (ξ , 𝒟) (drop id) ℰ ⁻¹
                 ⋮ (\ ξ′ → sub ξ′ ℰ) & id-gets ξ


expand-subs : ∀ {Γ Ξ Ψ A} → (ξ : Γ ⊢ Ξ true*) (𝒟 : Γ ⊢ A true) (ψ : Ξ ⊢ Ψ true*)
...
comp-ren-sub : ∀ {Γ Γ′ Ξ A} → (η : Γ′ ⊇ Γ) (ξ : Γ ⊢ Ξ true*) (𝒟 : Ξ ⊢ A true)
...
comp-sub : ∀ {Γ Ξ Ψ A} → (ξ : Γ ⊢ Ξ true*) (ψ : Ξ ⊢ Ψ true*) (𝒟 : Ψ ⊢ A true)
...
lid-subs : ∀ {Γ Ξ} → (ξ : Γ ⊢ Ξ true*)
                   → subs ids ξ ≡ ξ
lid-subs ∙       = refl
lid-subs (ξ , 𝒟) = _,_ & lid-subs ξ ⊗ id-sub 𝒟


rid-subs : ∀ {Γ Ξ} → (ξ : Γ ⊢ Ξ true*)
                   → subs ξ ids ≡ ξ
rid-subs ∙       = refl
...
assoc-subs : ∀ {Γ Ξ Ψ Φ} → (ξ : Γ ⊢ Ξ true*) (ψ : Ξ ⊢ Ψ true*) (φ : Ψ ⊢ Φ true*)
...
instance
  𝐈𝐏𝐋 : Category (List Prop) _⊢_true*
  𝐈𝐏𝐋 = record
          { id     = ids
          ; _∘_    = flip subs
          ; lid∘   = rid-subs
