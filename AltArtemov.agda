...
data _∶ⁿ_~_ : ∀{m} → VVar m → Cx m → Cx m → Set where
  base : ∅ ∶ⁿ ∅ ~ ∅
...
in⊢ 𝐱 P (R𝜆ⁿ {n} {𝐱 = 𝐲} {𝐭} D)
    = let xₘ₊₁      = fresh
          ⟨ s , C ⟩ = in⊢ (xₘ₊₁ ∷ 𝐱) (step P) D
      in
        ⟨ (λ 𝐱 → 𝜆^[ suc n ] xₘ₊₁ ． s (xₘ₊₁ ∷ 𝐱))
        , R𝜆ⁿ {𝐱 = xₘ₊₁ ∷ 𝐲} {𝐭 = s (xₘ₊₁ ∷ 𝐱) ∷ 𝐭} C
        ⟩

in⊢ 𝐱 P (R∘ⁿ {n} {𝐭} {𝐬} Dₜ Dₛ)
    = let ⟨ sₜ , Cₜ ⟩ = in⊢ 𝐱 P Dₜ
          ⟨ sₛ , Cₛ ⟩ = in⊢ 𝐱 P Dₛ
      in
        ⟨ (λ 𝐱 → sₜ 𝐱 ∘^[ suc n ] sₛ 𝐱)
...
_∶ⁿ_ : ∀{m} → VVar m → Cx m → Cx m
𝐱 ∶ⁿ Γ = zipWith _∶_ (map 𝜈_ 𝐱) Γ


ind~ : ∀{m} {𝐱 : VVar m} {Γ : Cx m} → 𝐱 ∶ⁿ Γ ~ (𝐱 ∶ⁿ Γ)
ind~ {𝐱 = ∅}     {Γ = ∅}     = base
ind~ {𝐱 = x ∷ 𝐱} {Γ = A ∷ Γ} = step (ind~ {𝐱 = 𝐱} {Γ = Γ})


int : ∀{B m} {𝐱 : VVar m} {Γ : Cx m}
    → Γ ⊢ B
    → Σ (VVar m → Tm) (λ t → 𝐱 ∶ⁿ Γ ⊢ t 𝐱 ∶ B)
int {𝐱 = 𝐱} D = in⊢ 𝐱 ind~ D


ind≲ : ∀{m} {Γ : Cx m} → ∅ ≲ Γ
ind≲ {Γ = ∅}     = base
ind≲ {Γ = x ∷ Γ} = drop (ind≲ {Γ = Γ})
...
eI²′ : ∀{x A m} {Γ : Cx m}
     → Σ Tm (λ t → Γ ⊢ t ∶ 𝜆 x ． 𝜈 x ∶ (A ⊃ A))
eI²′ = nec eI

eI³′ : ∀{u x A m} {Γ : Cx m}
     → Σ Tm (λ t → Γ ⊢ t ∶ 𝜆² u ． 𝜈 u ∶ 𝜆 x ． 𝜈 x ∶ (A ⊃ A))
eI³′ = nec eI²
