module A201605.AltArtemov.Old.Common.True.WithG where

open import A201605.AltArtemov.Old.Common.Var.WithG public


data True (Γ : Cx) : ∀ {n} → Ty 0 n → Set where
  var  : ∀ {n} {A : Ty 0 n} → Var Γ A → True Γ A
  lam  : ∀ {n} {A B : Ty 0 n} → True (Γ , A) B → True Γ (A ⊃ B)
  app  : ∀ {n} {A B : Ty 0 n} → True Γ (A ⊃ B) → True Γ A → True Γ B
  pair : ∀ {n} {A B : Ty 0 n} → True Γ A → True Γ B → True Γ (A ∧ B)
  fst  : ∀ {n} {A B : Ty 0 n} → True Γ (A ∧ B) → True Γ A
  snd  : ∀ {n} {A B : Ty 0 n} → True Γ (A ∧ B) → True Γ B
--  up   : ∀ {n} {t : Tm 0 n} {A : Ty 0 n} → True Γ (t ∶ A) → True Γ (! t ∶ t ∶ A)
--  down : ∀ {n} {t : Tm 0 n} {A : Ty 0 n} → True Γ (t ∶ A) → True Γ A

ᵗ⌊_⌋ : ∀ {Γ n} {A : Ty 0 n} → True Γ A → Tm ᵍ⌊ Γ ⌋ n
ᵗ⌊ var x ⌋      = VAR ⁱ⌊ x ⌋
ᵗ⌊ lam j ⌋      = LAM ᵗ⌊ j ⌋
...
ren-true : ∀ {Γ Γ′ n} {A : Ty 0 n} → Γ′ ⊇ Γ → True Γ A → True Γ′ A
...
wk-true : ∀ {Γ n n′} {A : Ty 0 n} {C : Ty 0 n′} → True Γ C → True (Γ , A) C
wk-true = ren-true ⊇wk

wk*-true : ∀ {Γ n} {C : Ty 0 n} → True ∅ C → True Γ C
wk*-true = ren-true ⊇to

ren-true-id : ∀ {Γ n} {A : Ty 0 n} (j : True Γ A) → ren-true ⊇id j ≡ j
...
ren-true-● : ∀ {Γ Γ′ Γ″ n} {A : Ty 0 n} (η′ : Γ″ ⊇ Γ′) (η : Γ′ ⊇ Γ) (j : True Γ A) →
...
v₀ : ∀ {Γ n} {A : Ty 0 n} → True (Γ , A) A
v₀ = var x₀

v₁ : ∀ {Γ n n′} {A : Ty 0 n} {B : Ty 0 n′} → True ((Γ , A) , B) A
v₁ = var x₁

v₂ : ∀ {Γ n n′ n″} {A : Ty 0 n} {B : Ty 0 n′} {C : Ty 0 n″} → True (((Γ , A) , B) , C) A
v₂ = var x₂

I : ∀ {Γ n} {A : Ty 0 n} → True Γ (A ⊃ A)
I = lam v₀

K : ∀ {Γ n} {A B : Ty 0 n} → True Γ (A ⊃ B ⊃ A)
K = lam (lam v₁)

S : ∀ {Γ n} {A B C : Ty 0 n} → True Γ ((A ⊃ B ⊃ C) ⊃ (A ⊃ B) ⊃ A ⊃ C)
S = lam (lam (lam
      (app (app v₂ v₀)
           (app v₁ v₀))))
