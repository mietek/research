module AltArtemov.Common.OPE.WithG where

open import AltArtemov.Common.Cx.WithG public


data _⊇_ : Cx → Cx → Set where
  base : ∅ ⊇ ∅
  weak : ∀ {Γ Γ′ n} {A : Ty 0 n} → Γ′ ⊇ Γ → (Γ′ , A) ⊇ Γ
  lift : ∀ {Γ Γ′ n} {A : Ty 0 n} → Γ′ ⊇ Γ → (Γ′ , A) ⊇ (Γ , A)
...
⊇wk : ∀ {Γ n} {A : Ty 0 n} → (Γ , A) ⊇ Γ
⊇wk = weak ⊇id

⊇str : ∀ {Γ Γ′ n} {A : Ty 0 n} → Γ′ ⊇ (Γ , A) → Γ′ ⊇ Γ
⊇str (weak η) = weak (⊇str η)
⊇str (lift η) = weak η

⊇drop : ∀ {Γ Γ′ n} {A : Ty 0 n} → (Γ′ , A) ⊇ (Γ , A) → Γ′ ⊇ Γ
⊇drop (weak η) = ⊇str η
⊇drop (lift η) = η
...
°ren-ty-id : ∀ {Γ n} (A : Ty ᵍ⌊ Γ ⌋ n) → ren-ty ʰ⌊ ⊇id ⌋ A ≡ A
°ren-ty-id {Γ} rewrite °id Γ = ren-ty-id
