...
open LG using () renaming (_⨾_⊢_⦂_ to LG⟨_⨾_⊢_⦂_⟩ ; _⊢_≤_ to LG⟨_⊢_≤_⟩) public
...
module Local where
  ...
  box±± : ∀ {x A Γ} Γ′ {Ξ}
          → Γ ⨾ Ξ ⊢ x ⦂ A
          → Γ ±± Γ′ ⨾ Ξ ⊢ x ⦂ □ A
  box±± {x} Γ′ t = scan λ {y} → mono⊢ (weak⊆±±ᴸ Γ′) (box-lem t)
...
module Nonlocal where
  ...
  box±± : ∀ {x y A Γ} Γ′ {Ξ}
          → Γ ⨾ Ξ , x ∙ y ⊢ y ⦂ A
          → Γ ±± Γ′ ⨾ Ξ , x ∙ y ⊢ y ⦂ □ A
  box±± {x} {y} Γ′ t = scan λ {z} → mono⊢ (weak⊆±±ᴸ Γ′) (box-lem t)
...
g→lg₁ (G.var i)      = LG.var (mono∈ weak⊆±±ᴿ (→var i))
g→lg₁ (G.lam t)      = LG.lam (g→lg₁ t)
g→lg₁ (G.app t u)    = LG.app (g→lg₁ t) (g→lg₁ u)
g→lg₁ {x} {Γ} (G.mvar i)    = Local.mvar (mono∈ (weak⊆±±ᴸ (la x Γ)) (→mvar i))
...
g→lg₂ (G.lam t)      = LG.lam (g→lg₂ t)
g→lg₂ (G.app t u)    = LG.app (g→lg₂ t) (g→lg₂ u)
g→lg₂ {y} {Γ} (G.mvar i)    = Nonlocal.mvar (mono∈ (weak⊆±±ᴸ (la y Γ)) (→mvar i))
