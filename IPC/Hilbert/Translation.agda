...
lc→tc (Ξ , ts) = lc×→tc ts top
  where
    ...
    lc×→tc (LC.unit ts)   (pop k) = lc×→tc ts k
...
l→lc₀ (Ξ , ts) = Ξ , l×→lc₀× ts
  where
    ...
    l×→lc₀× (L.unit ts)   = LC.unit (l×→lc₀× ts)
...
lc₀→l (Ξ , ts) = Ξ , lc₀×→lc× ts
  where
    ...
    lc₀×→lc× (LC.unit ts)   = L.unit (lc₀×→lc× ts)
